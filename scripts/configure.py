...
def write_configs(host, user, key_path, pubkey_path, pool_name, net_name, mac_addr, mode="direct"):
...
TARGET_MAC="{mac_addr}"
TARGET_DEPLOYMENT_MODE="{mode}"
""")
    print(f"  {GREEN}✓ Generated:{RESET} target.env")

    # 2. Stage 0 (Sandbox Hypervisor) terraform.tfvars
    stage0_tfvars_path = os.path.join(REPO_ROOT, "terraform", "environments", "00-sandbox-hypervisor", "terraform.tfvars")
    with open(stage0_tfvars_path, "w") as f:
        f.write(f"""# Generated by 'make configure'
...
""")
    print(f"  {GREEN}✓ Generated:{RESET} terraform/environments/00-sandbox-hypervisor/terraform.tfvars")

    # 3. Stage 1 (Talos Cluster) terraform.tfvars (if direct mode)
    if mode == "direct":
        ...
        with open(stage1_tfvars_path, "w") as f:
            f.write(f"""# Generated by 'make configure' (Direct Target Mode)
libvirt_uri  = "{libvirt_uri}"
storage_pool = "{pool_name}"
network_name = "{net_name}"
""")
        print(f"  {GREEN}✓ Generated:{RESET} terraform/environments/01-talos-cluster/terraform.tfvars (Direct Mode)")

def main():
    parser = argparse.ArgumentParser(description="Target Server Configuration & Verification Wizard")
    parser.add_argument("--mode", choices=["direct", "nested-sandbox"], default=None, help="Target deployment mode: direct (default) or nested-sandbox")
    ...
    if not mode:
        if args.non_interactive:
            ...
            print(f"{BLUE}===> Select Target Deployment Strategy:{RESET}")
            print(f"  [1] Direct Target: Deploy cluster directly onto target hypervisor (Dell T5600, Proxmox, Local KVM)")
            print(f"  [2] Nested Sandbox (Stage 0): Deploy cluster inside a dedicated sandbox VM hypervisor")
            choice = prompt_value("Select deployment mode (1 or 2)", "1")
            mode = "nested-sandbox" if choice == "2" else "direct"

    # 2. Determine host alias
    if args.alias:
        ...
        prompt_label = "Base Host SSH Alias (running Stage 0 VM)" if mode == "nested-sandbox" else "Target Server SSH Host Alias (e.g. homelab1, k8s-host, t5600)"
        host_alias = prompt_value(prompt_label).strip()
    ...
    print(f"\n{BLUE}===> Persisting verified target configuration ({mode} mode)...{RESET}")
    write_configs(host, user, key_path, pubkey_path, pool_name, net_name, mac_addr, mode=mode)

    print(f"\n{GREEN}{BOLD}🎉 Deployment configuration saved for '{remote_host}' ({host}) in {mode.upper()} mode!{RESET}")
    if mode == "nested-sandbox":
        print(f"{GREEN}Next Steps: Run 'make stage0-apply' to spin up the Sandbox VM. It will automatically wire the cluster target.{RESET}\n")
    else:
        print(f"{GREEN}Next Steps: Target is ready! You can proceed directly to 'make up' or 'make stage1-apply'.{RESET}\n")
    return 0
